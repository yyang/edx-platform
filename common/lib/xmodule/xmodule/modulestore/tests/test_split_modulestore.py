...
class SplitModuleTest(unittest.TestCase):
    ...
    def bootstrapDB():
        ...
        to_publish = BlockUsageLocator(
            CourseLocator(org="testx", course="course", run="test_run", branch=BRANCH_NAME_DRAFT),
            block_type='course',
            block_id="head23456"
        )
        destination = CourseLocator(org="testx", course="course", run="test_run", branch=BRANCH_NAME_PUBLISHED)
...
class SplitModuleItemTests(SplitModuleTest):
    ...
    def test_has_item(self):
        ...
        org = 'testx'
        offering = 'GreekHero'
        course_locator = CourseLocator(org=org, course=offering, run="run", branch=BRANCH_NAME_DRAFT)
